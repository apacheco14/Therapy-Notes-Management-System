...
{
	/**
	 * Checks if a client_code already exists in the database. Used by the Service layer to prevent duplicate clinical records.
	 */
	public boolean existsByCode( String clientCode ) throws SQLException
	{
		...
		AppLogger.logDatabaseOperation( "SELECT COUNT(*)", "clients", true );
		return false;
	}
			...
			throws SQLException, TherapyAppException
	{
		...
		AppLogger.logDatabaseOperation( "SELECT", "clients", true );
		return clients;
	}
